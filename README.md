...
Community integration for local **Laya + Codex / Claude Code / Cursor / ChatGPT**. Based on upstream [Laya](https://github.com/NandhaKishorM/laya), whose documentation and credits follow below.

**网页 GPT 接入：** 请阅读 [CHATGPT-SETUP.md](CHATGPT-SETUP.md)。下载到独立的 `laya` 文件夹后，运行 `python start_chatgpt.py`，按提示连接官方 MCP Tunnel。首次需要用户的 ChatGPT 开发者模式、工作区 tunnel 和本机密钥输入；读取 GitHub 不会自动安装或授予账号权限。
...
cd laya
python start_chatgpt.py
```

AI assistants: when the user asks to install or connect this repository, read [CHATGPT-SETUP.md](CHATGPT-SETUP.md) for the web ChatGPT flow. Confirm the execution environment and available tools; verify actual tool calls before reporting a successful connection.

Community: [Contributing](CONTRIBUTING.md) · [Discussions](https://github.com/Maxwell00000086/laya-agent-kit/discussions) · [Report a bug](https://github.com/Maxwell00000086/laya-agent-kit/issues/new/choose) · [Security policy](SECURITY.md) · [Apache-2.0 license](LICENSE).
...
For Codex, Claude Code, Cursor, or another local MCP client, this checkout includes a community integration installer: `python install.py --client codex --client claude-code`. See [Laya Agent Kit](agent-kit/README.md) or the [Chinese setup guide](AGENT-KIT.md). This paragraph is an addition to the upstream README; the model library below remains upstream Laya.
